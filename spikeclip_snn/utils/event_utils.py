...
def load_events(file_path: str) -> np.ndarray:
    ...
    #     x = (data >> 32) & 0xFF  # bits 39-32 (8 bits)
    #     y = (data >> 24) & 0xFF  # bits 31-24 (8 bits)
    #     polarity_bit = (data >> 23) & 0x01  # bit 23 (1 bit)
    #     timestamp = data & 0x7FFFFF  # bits 22-0 (23 bits)
    #
    #     # Convert polarity: 0 → -1 (OFF), 1 → +1 (ON)
    #     polarity = 1 if polarity_bit == 1 else -1
    #
    #     # Add event [x, y, timestamp, polarity]
    #     events.append([x, y, timestamp, polarity])
